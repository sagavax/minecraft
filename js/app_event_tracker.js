const events = ['click', 'keydown', 'scroll', 'input', 'keyup', 'mousemove'];
var module = sessionStorage.getItem("current_module");
...
function checkLastEvent() {
    ...
    xhttp.open("GET", "app_check_event.php", true);
    xhttp.send();
}

function writeEvent(event) {
    var xhttp = new XMLHttpRequest();
    var current_module = sessionStorage.getItem("current_module");
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
            if (this.status != 200) {
                // Handle errors here
    ...
    xhttp.open("POST", "app_capture_event.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    var data = "event=" + encodeURIComponent(event)+"&current_module="+current_module;
    xhttp.send(data);
}
