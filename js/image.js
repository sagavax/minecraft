...
modal_new_tags.addEventListener("click", function(event) {
  if (event.target.tagName === "BUTTON" && event.target.name == "add_new_tag") {
    ...
    // Získaj nový tag zo zvoleného tlačidla
    var newTag = event.target.innerText;

    // Skontroluj duplicitu
    if (tagsArray.includes(newTag)) {
      alert("Tag already exists.");
      return;  // Ak existuje, prerušíme funkciu
    } else {
      var tagId = event.target.getAttribute("tag-id");
      console.log("New tag added: ", tagId);

      // Pridaj tag do obrázku (ak je to potrebné)
      addTagToImage(tagId);

      // Ak chceš pridať nový tag do zoznamu na stránke
      tagsArray.push(newTag);

      // Ulož nový zoznam tagov späť do atribútu
      document.querySelector(".images_tags").setAttribute("tag-list", JSON.stringify(tagsArray));

      // (Voliteľné) Ak chceš zmeniť stav na serveri alebo niečo podobné:
      // saveNewTagsToServer(tagsArray);
    }
  } 

  if(event.target.tagName === "BUTTON" && event.target.name == "letter"){
...
// Select the element that contains the picture comments
 ...
 function saveImageName(new_name){
   var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
       if (this.readyState == 4 && this.status == 200) {
