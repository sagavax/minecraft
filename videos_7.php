<?php include "includes/dbconnect.php";
      ...
      if(isset($_POST['add_new_video'])){
        ...
        if(isset($_POST['add_to_favorites'])){
          ...
          $sql="UPDATE videos set is_favorite=1 where video_id=1";
          ...
          //$link1=mysqli_connect("localhost", "root", "", "brick_wall");
          $curr_date=date('Y-m-d H:i:s');
          $diary_text="Minecraft IS: video s nazvom <strong>$video_name</strong> bolo zaradene medzi oblubene videa";
          $sql="INSERT INTO diary (diary_text, date_added,location,isMobile,is_read) VALUES ('$diary_text','$curr_date','',0,0)";
          $result = mysqli_query($link1, $sql) or die("MySQLi ERROR: ".mysqli_error($link1));
          mysqli_close($link1);

          echo "<script>altert('Video s id $video_id zaradene medzi oblubene videa')</script>";
          $result = mysqli_query($link1, $sql) or die("MySQLi ERROR: ".mysqli_error($link1));
        }
        ...
        if(isset($_POST['remove_from_favorites'])){
          ...
          echo "<script>altert('Video s id $video_id bolo vyradene z oblubenych videi')</script>";
          $result = mysqli_query($link1, $sql) or die("MySQLi ERROR: ".mysqli_error($link1));
        }
?>      
<!DOCTYPE html>
          ...
          <div class='list'>
            ...
            <div id="video_list">
                ...
                <?php
                    ...
                    $sql="SELECT video_id, video_title, eis_video_id, video_url, cat_id, modpack_id, is_favorite from videos a ORDER BY video_id DESC";
                    }
                    //echo $sql;
                    $result=mysqli_query($link, $sql);
                        while ($row = mysqli_fetch_array($result)) {
